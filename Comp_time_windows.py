...
"""
Fill out dictionaires and tracking for all the weeks since the tracking start date (weekly)
"""
...
meta_dict = {}
"""
parse SRA metadata to get accessions for samples collected during each tracked week
"""
...
# get accession of already processed samples so they won't be reprocessed
read_acc = []
...
file_dict = {}
cram_dict = {}
# check for new samples to process
new = 0
for week in weeks:
    file_dict[week] = []
    cram_dict[week] = []
...
if file_search:
    """
    look through directories in priority order for outputs for unprocessed samples. skips repeat outputs
    priority should no longer be relevant
    """
    ...
    for path in paths:
        ...
        for subdir, dirs, files in os.walk(path):
            for file in files:
                if file.endswith(".cut_covars.tsv.gz"): # MUST BE GZIPPED
                    acc = file.split(".")[0]
                    for week in weeks:
...
def proc_week(week):
    """
    Called to get the variant (PM) from covars and coverage from nt call
    Parameters:
    week - week to process the samples from
    Functionality:
    iterates though the samples of the week, pulling data from the SAM Refiner outputs to generate polymorphism and coverage data

    NO Returns, generats or updates local covars and nts files prefixed with the week.  prints samples with incomplete outputs
    """
    if file_dict[week]:
        ...
        for file in file_dict[week]:
            ...
            acc = sfile[-1].split(".")[0]
            path = "/".join(sfile[:-1])
            # MUST BE GZIPPED, required both covar and nt call outputs
...
def rs(week):
    """
    Called to consolidate the week's covar and nts data
    Parameters:
    week - week to process the samples from
    Functionality:
    collects sample specific data in each file and rewrites file with just totals for the week
    NO Returns, updates local covars and nts files prefixed with the week.
    """
...
JN_changes = []
JN_AAchanges = {}
# get the covars for JN.1 to be ignored
...
JN_clade_PM_dict = {}
# pull varient information generated by Tree.py
...
ranked_vars = []
"""
Get the abundance of ciculating lineages so they can have ranked reporting 
WHO isn't actually useful as long as CDC maintains its reporting
"""
if update_cir == "WHO":
...
def weekly_out(j):
    ...
    Parameters:
    j - week to anchor the windows for time compairison as the last week of the 6
    Functionality:
    Collects vairant data in each timeframe and check for changes that pass the threshold to be reported
    ...
    with open(f"{weeks[j]}.VariantPMs.tsv", "w") as var_out:
        ...
        for pos in sorted(pm_dict.keys()):
            ...
            for change in set(changes):
                """
                splits up PM information pulled from covar.tsv to generate seperate columns for the output
                then adds the statistical info
                """
                orfs = []
                AAchanges = []
